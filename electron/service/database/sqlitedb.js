...
class SqlitedbService extends BasedbService {
  ...
  init() {
    ...
    if (!tableExists) {
      // 创建表
      const create_user_table_sql = `CREATE TABLE ${this.chatMessagesTableName}
      (
  ...
  async addChatMessage(data) {
    const insert = this.db.prepare(
      `INSERT INTO ${this.chatMessagesTableName} (nickname, content) VALUES (@nickname, @content)`
    )
    insert.run(data)
    return true
  }
  ...
  async updateChatMessage(data) {
    const updateInfo = this.db.prepare(
      `UPDATE ${this.chatMessagesTableName} SET nickname = @nickname, content = @content WHERE id = @id`
    )
    updateInfo.run(data)
    return true
  }
  ...
  async getChatMessage(keyword = '') {
    const selectUser = this.db.prepare(
      `SELECT * FROM ${this.chatMessagesTableName} WHERE content = @content`
    )
