<template>
  <div class="relative">
    <RecycleScroller
      ...
      :items="dataList"
      :item-size="56"
      key-field="channel_id"
      @resize="onResize"
      @scroll="onScroll"
    >
      <template #default="{ item }">
        <div class="flex p-2 cursor-pointer rounded-md hover:bg-[#f0f0f0]" @click="handleClick">
          <Avatar :src="avatarUser(item.channel_id, channelType)" shape="circle" :size="40" />
          <div class="flex-1 pl-3">
            <div class="flex items-center leading-[40px]" v-html="item.channel_name"></div>
          </div>
        </div>
      </template>
      <template #after>
...
import { avatarUser, newChannel } from '@/wksdk/channelManager'
...
const channelType = computed(() => {
  return props.resultField === 'friends' ? ChannelTypePerson : ChannelTypeGroup
})
...
const handleClick = (item) => {
  console.log(item)
  const channel = newChannel(item.channel_id, item.channel_type)
